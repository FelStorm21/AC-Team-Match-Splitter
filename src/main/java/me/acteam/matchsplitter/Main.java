...
//		fileSplitter.writeQuestions();
		...
		DirectoryIterator dir = new DirectoryIterator("C:\\Users\\bzall\\OneDrive\\Documents\\AC Team Matches\\2014-15"); //set default path
		
		if (args.length != 0) {
			if (args[0].equals("-gui")) {
				GUI gui = new GUI();
				if (gui.didSelect()) {
					dir.setRootPath(gui.getSelectedFile().toPath());
				} else {
					System.out.println("File selection unsuccessful.");
					isRunning = false;
				}
			} else if (args[0].equals("-path")){
				dir.setRootPath(Paths.get(args[1]));
			} else {
